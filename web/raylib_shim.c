...
void cf_draw_line_ex(float start_x, float start_y, float end_x, float end_y,
                     float thick, unsigned char r, unsigned char g,
...
static int cf_sound_valid_handle(int h) { return h >= 0 && h < cf_sound_count; }

int cf_load_sound(const char *path) {
  if (cf_sound_count >= CF_MAX_SOUNDS)
    return -1;
...
void cf_play_sound(int h) {
  if (cf_sound_valid_handle(h))
    PlaySound(cf_sounds[h]);
}

void cf_stop_sound(int h) {
  if (cf_sound_valid_handle(h))
    StopSound(cf_sounds[h]);
}

void cf_pause_sound(int h) {
  if (cf_sound_valid_handle(h))
    PauseSound(cf_sounds[h]);
}

void cf_resume_sound(int h) {
  if (cf_sound_valid_handle(h))
    ResumeSound(cf_sounds[h]);
}
...
void cf_set_sound_volume(int h, float volume) {
  if (cf_sound_valid_handle(h))
    SetSoundVolume(cf_sounds[h], volume);
}

void cf_set_sound_pitch(int h, float pitch) {
  if (cf_sound_valid_handle(h))
    SetSoundPitch(cf_sounds[h], pitch);
}

void cf_set_sound_pan(int h, float pan) {
  if (cf_sound_valid_handle(h))
    SetSoundPan(cf_sounds[h], pan);
}

void cf_unload_sound(int h) {
  if (cf_sound_valid_handle(h))
    UnloadSound(cf_sounds[h]);
}
